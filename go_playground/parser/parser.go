...
func Parse(input string) (_ Expr, err error) {
	...
	if lex.token != scanner.EOF {
		return nil, fmt.Errorf("unexpected %s", lex.desc())
	}
	return e, nil
}
...
func parsePrimary(lex *lexer) Expr {
	...
	case scanner.Ident:
		...
		if lex.token != ')' {
			...
			if lex.token != ')' {
				panic(lexPanic(fmt.Sprintf("got %q, expect ')'", lex.token)))
			}
		}
		lex.next()
		return call{id, args}
	...
	case '(':
		...
		e := parseExpr(lex)
		if lex.token != ')' {
			panic(lexPanic(fmt.Sprintf("got %s, expect ')'", lex.desc())))
		}
		lex.next()
		return e
	}
	panic(lexPanic(fmt.Sprintf("unexpected %s", lex.desc())))
}
